...
tag=lambda ax,t: ax.text(-0.12,1.10,t,transform=ax.transAxes,fontsize=14,fontweight="bold",va="top")
...
ax.set_xlabel("noise η"); ax.set_ylabel("σ at φ = 0.90")
ax.set_title("Descriptive threshold crossing"); ax.set_xlim(min(es)-0.08,max(es)+0.08); tag(ax,"b")
...
ax.set_xlabel("trait dispersion σ"); ax.set_ylabel("N × variance of run means")
ax.set_title("Between-run variability"); ax.legend(); tag(ax,"c")
...
ax.set_title("Temporal persistence"); ax.legend(loc="lower right"); tag(ax,"d")

ax=fig.add_subplot(gs[1,1])
d5=summ[summ.noise==0.5].sort_values("sigma_std")
ax.plot(d5.sigma_std,100*d5.frac_lo,"-o",color="#c0392b",ms=3.5,mec="white",mew=0.5,label="low response (w<0.2)")
ax.plot(d5.sigma_std,100*d5.frac_hi,"-s",color="#27ae60",ms=3.5,mec="white",mew=0.5,label="high response (w>0.8)")
ax.set_xlabel("trait dispersion σ"); ax.set_ylabel("% of population (η=0.5)")
ax.set_title("Trait tail fractions"); ax.legend(loc="upper left"); tag(ax,"e")
...
if HAVE_SCIPY:
    ...
    ax.set_ylim(0,6); ax.set_xlabel("alignment response w"); ax.set_ylabel("trait density")
    ax.set_title("Beta distribution shapes"); ax.legend()
else:
    ax.text(0.5,0.5,"(install scipy for this panel)",ha="center",va="center",transform=ax.transAxes); ax.set_axis_off()
tag(ax,"f")

fig.suptitle(os.environ.get("ABM_DATA_LABEL", "Legacy approximate-search model: heading order and response dispersion"),fontsize=14,fontweight="bold",y=1.00)
